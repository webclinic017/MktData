...
FORCE_LIVE = True
GENERAL_FEATURES=[ 'Code','CountryName','Sector','InternationalDomestic','Industry','GicSector','GicIndustry','GicSubIndustry','HomeCategory']

def get_fundamental_data(symbol= 'AAPL.US', api_token='5d9220f54d3479.22613861', session=None, live = FORCE_LIVE ):
    if( not live ):
...
def get_index_tickers(index = "GSPC", initdate = '2020-01-01' ,country='US'):
    #init date is to avoid lookforward bias by selecting universe as index constituints as of today, which 
    #will include names that were listed, and exclude names that were delisted
    #right way to do it is to train the model on a dynamic set of panel names
    #which corresponds to index members at the training day - 
    #but that's not feasible - so we just fix the portfolio as of the starting date for model training
    #and then for backtesting, make the portfolio selection dynamical
...
def sharpe_ratio(rets, risk_free=0):
    rets = rets.replace(np.inf,0)
...
def get_exchange_data(symbol= 'US', api_token='5d9220f54d3479.22613861', session= None, live = FORCE_LIVE ):
    if( not live ):
...
def parse_dates( df, use_filing_date = False ):   
    if df.empty:
...
def financials_postprocess( data, filing_type, use_filing_date = True ):
...
def compute_market_cap( history, shares ):
...
def get_realized_volatility( rets, days, meanadj = 1 , weekends = False):
    #adding the realized volatility swap contract, mean-adjusted
    #probably other measures of probability will be better, using the simplest one for now
...
def get_momentum( data, days_short = 30, days_long = 365 ):
    momentum = np.log(get_moving_average( data, days_short ) / get_moving_average( data, days_long ))
    return( momentum )


def get_fund_df( f_history, mktcap_history, ratios, extrapolate_forward = True ):
...
def get_sheet( df_factors, factor, periods, quantiles = 5, ticker_key = '__panel' ):
...
class MktObj:
    ...
    def History( self,  init_date="2008-01-01", final_date="2030-12-12", resolution = 'd', refresh = True ):
    ...
    def IntradayHistory( self, interval = '5m', refresh = True ):
...
class Stock( MktObj ):
    ...
    def Raw_Financial_History( self, refresh = True  ):
    ...
    def General_Info( self, item = None ):
